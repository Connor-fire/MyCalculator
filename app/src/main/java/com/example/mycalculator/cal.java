...
public class cal {
    ...
    public static String bracketsString(StringBuffer str){
        int leftBracketsPoint=str.lastIndexOf("(");//返回指定字符在此字符串中最后一次出现处的索引，如果此字符串中没有这样的字符，则返回 -1。
        int rightBracketsPoint=str.indexOf(")");//返回指定字符在字符串中第一次出现处的索引，如果此字符串中没有这样的字符，则返回 -1。
        if(leftBracketsPoint==-1 || rightBracketsPoint==-1){
            return null;
        }
        String res= str.substring(leftBracketsPoint+1,rightBracketsPoint);//返回两括号间的字符串beginIndex -- 起始索引（包括）, 索引从 0 开始。endIndex -- 结束索引（不包括）。
        str.replace(leftBracketsPoint, rightBracketsPoint+1, "@");//把左右括号替换
        return res;
    }
